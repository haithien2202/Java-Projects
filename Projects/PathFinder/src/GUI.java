...
public abstract class GUI {
	...
	protected static String [] stopList = {"Distance","Time"};
	protected JComboBox<String> StopList = new JComboBox<String>(stopList);
	protected JCheckBox checkList = new JCheckBox("Not for Bike");
	protected JCheckBox checkList2 = new JCheckBox("Not for Car");
	protected JCheckBox checkList3 = new JCheckBox("Not for Pedestrian");
	protected JCheckBox checkList4 = new JCheckBox("Avoid traffic light");
	/**
	...
	protected abstract void onRClick(MouseEvent e);
	
	protected abstract void searchArt();
	...
	protected abstract void filterByDistance();
	
	
	//add filter for the road
	
	protected abstract void noCar();
	
	protected abstract void noPede();
	
	protected abstract void noBike();
	
	
	//remove filter for the road
	
	protected abstract void forCar();
	
	protected abstract void forPede();
	
	protected abstract void forBike();
	
	protected abstract void trafficLightOn();
	
	protected abstract void trafficLightOff();
	
	
	protected abstract Point onDrag(MouseEvent e, double x2 , double y2);
	
	protected abstract void onScroll(boolean m);
	...
	protected abstract void onLoad(File nodes, File roads, File segments,
			File polygons, File trafficLight);
	...
	private static final String TRAFFIC_LIGHT = "traffic-lights.tab";
	...
	private void initialise() {
		...
		load.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				File nodes = null, roads = null, segments = null, polygons = null, trafficLight = null;
				...
				if (fileChooser.showOpenDialog(frame) == JFileChooser.APPROVE_OPTION) {
					...
					for (File f : files) {
						if (f.getName().equals(NODES_FILENAME)) {
							...
							polygons = f;
						} else if (f.getName().equals(TRAFFIC_LIGHT)) {
							trafficLight = f;
						}
					}
					...
					if (nodes == null || roads == null || segments == null) {
						...
						onLoad(nodes, roads, segments, polygons, trafficLight);
						redraw();
					}
				}
			}
		...
		JButton artBut = new JButton("Articulation Point");
		artBut.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				searchArt();
				redraw();
			}
		...
		JPanel artP = new JPanel();
		artP.setLayout(new GridLayout(1, 1));
		artP.add(artBut);
		artP.setMaximumSize(new Dimension(50, 100));
		controls.add(artP);
		controls.add(Box.createRigidArea(new Dimension(10, 0)));
		controls.add(new JLabel("Search Filter"));
		controls.add(Box.createRigidArea(new Dimension(10, 0)));
		StopList.setMaximumSize(new Dimension(50,25));
		...
		controls.add(search);
		controls.add(StopList);
		controls.add(checkList);
		checkList.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ev) {
				if (checkList.isSelected() == true) noBike();
				else forBike();
		}	
		});
		controls.add(checkList2);
		checkList2.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ev) {
				if (checkList2.isSelected() == true) noCar();
				else forCar();
		}	
		});
		controls.add(checkList3);
		checkList3.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ev) {
				if (checkList3.isSelected() == true) noPede();
				else forPede();
		}	
		});
		controls.add(checkList4);
		checkList4.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent ev) {
				if (checkList4.isSelected() == true) trafficLightOn();
				else trafficLightOff();
				redraw();
		}	
		});
