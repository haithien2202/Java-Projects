...
public abstract class AbstractCardGame implements CardGame {
	...
	public AbstractCardGame(AbstractCardGame original) {
		for(Player.Direction d : Player.Direction.values()) {
			this.players.put(d, original.players.get(d).clone());
			this.tricks.put(d, original.tricks.get(d));
			this.scores.put(d, original.scores.get(d));
		}
		this.trumps = original.trumps;
		this.currentTrick = (original.currentTrick == null) ? null : original.currentTrick.clone();
	}
