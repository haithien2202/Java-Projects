...
public abstract class GUI {
	...
	protected static String [] stopList = {""};
	...
	protected abstract void onCentral();
	...
	protected abstract Point onDrag(MouseEvent e, int x, int y);
	
	protected abstract void onScroll(boolean positive);
	...
	protected JTextArea textOutputArea;
	...
	private void initialise() {
		...
		fileChooser = new JFileChooser();
		JButton displayBut = new JButton("Display Quadtree");
		displayBut.setMaximumSize(new Dimension(200, 25));
		displayBut.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				onDisplay();
				redraw();
			}
		...
		if (UPDATE_ON_EVERY_CHARACTER) {
			...
			search.addKeyListener(new KeyAdapter() {
				public void keyReleased(KeyEvent e) {
					...
					//if (e.getKeyCode() == 8 || e.getKeyCode() == 127) return;
		...
		JButton centralBut = new JButton("Return centre");
		centralBut.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
		...
		JPanel quadtreeDisplay = new JPanel();
		quadtreeDisplay.setLayout(new GridLayout(1, 1));
		// manually set a fixed size for the panel containing the load and quit
		// buttons (doesn't change with window resize).
		quadtreeDisplay.setMaximumSize(new Dimension(200, 25));
		quadtreeDisplay.add(displayBut);
		controls.add(quadtreeDisplay);
		// rigid areas are invisible components that can be used to space
		// components out.
		controls.add(Box.createRigidArea(new Dimension(15, 0)));
		
		JPanel central = new JPanel();
		central.setLayout(new GridLayout(1, 1));
		central.add(centralBut);
		controls.add(centralBut);
		controls.add(Box.createRigidArea(new Dimension(10, 0)));
		
		JPanel searchBut = new JPanel();
		searchBut.setLayout(new GridLayout(1, 1));
		searchBut.setMaximumSize(new Dimension(250,27));
		searchBut.add(searchButton);		
