...
public class AvrModelChecker<T> {
  ...
  private Map<AbstractMemory, AVR.Memory> stateMap = new HashMap<>();
  ...
  public T apply(AbstractAvr seed) {
    ...
    while (worklist.size() > 0) {
      ...
      if (!stateMap.containsKey(state.getData())) {
    	  this.stateMap.put(state.getData(), state.getCode()); 
      }else if (stateMap.get(state.getData()) == state.getCode())
    	  break;
      //
      try {
