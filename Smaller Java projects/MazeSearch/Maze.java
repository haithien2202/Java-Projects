...
public class Maze {
    ...
    private ArrayList<directions> createRandomArray(){
           ArrayList<directions> directionsArray = new ArrayList<directions>(); 
           directionsArray.add(directions.NORTH);
           directionsArray.add(directions.SOUTH);
           directionsArray.add(directions.EAST);
           directionsArray.add(directions.WEST);
           Collections.shuffle(directionsArray);
    ...
    private void extendPathsFrom(MazeCell cell){
        cell.visit();
        ArrayList<directions> dirs = createRandomArray();
     ...
     */
    private MazeCell getNextCell(MazeCell cell, directions direction) {
        int row = cell.getRow();
        int col = cell.getCol();
        if (direction==directions.NORTH && row>0)           { return cells[row-1][col]; }
        else if (direction==directions.SOUTH && row<size-1) { return cells[row+1][col]; }
        else if (direction==directions.WEST  && col>0)      { return cells[row][col-1]; }
        else if (direction==directions.EAST  && col<size-1) { return cells[row][col+1]; }
        else                                       { return null; }
    }
