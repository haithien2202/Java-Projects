...
public class Earthquake implements Comparable<Earthquake> {
    ...
    public int compareTo(Earthquake other){
        for (int i = 0 ; i < this.ID.length() ; i++){
            if (this.ID.charAt(i) == other.ID.charAt(i)) continue;
            if (this.ID.charAt(i) > other.ID.charAt(i)) return 1;
            if (this.ID.charAt(i) < other.ID.charAt(i)) return -1;
            if ((this.ID.charAt(i) == other.ID.charAt(i)) && i == this.ID.length()) return 0; 
        }
        return 0;
    }  
}
