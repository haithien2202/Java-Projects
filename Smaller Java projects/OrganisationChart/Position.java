...
public class Position {
    ...
    private double spaceNeeded;                         // negative value to the left of the manager,
                            // positive value to the right

    // Constants for size of box representing the position
    public static final double WIDTH = 56;   
    public static final double HEIGHT = 40;  
    public static final double SPACE = 66;
    ...
    public double getSpace(){
        if (this.isManager() == false) { return SPACE; }   // this must be the top position
        double teamSpace = 0;
        Set<Position> team = this.getTeam();
        for (Position x : team ){
            teamSpace = teamSpace + x.getSpace();
        }
        return teamSpace;
    }
    ...
    public void drawDragging(double x, double y,boolean highlighted, boolean tempHighlighted,boolean check){
        // Background colour
        UI.setColor(tempHighlighted?TEMP_HIGHLIGHT_COL:(highlighted?HIGHLIGHT_COL:BACKGROUND_COL));
        ...
        UI.drawString((role==null)?"--":role, x+5, y+12);
        if (check == true){
              UI.setColor(Color.red);
              UI.setLineWidth(5);
              double off = 15;
              UI.drawLine((x+WIDTH/2 - off), (y+HEIGHT/2 - off), (x+WIDTH/2+off), (y+HEIGHT/2+ off));
              UI.drawLine((x+WIDTH/2 + off), (y+HEIGHT/2 - off), (x+WIDTH/2- off), (y+HEIGHT/2+off));
              UI.setLineWidth(1);
              UI.setColor(Color.black);
            }
